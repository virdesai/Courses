...
public class ScanString {
	...
	static NewTable<Object> table = new NewTable<Object>();
				...
				if (previous_space == count-1 && previous_space != 0) {
					...
					if (isNumeric(trimString(input.substring(previous_space,count))) == true) {
						...
						if (possibleWord.length() > 0) {
							...
							}else if (mainInput.toLowerCase().equals("failed") == true) {

								FailCT fail = new FailCT();
								fail.setInput(mainInput);
								big[arraySize] = fail;
								table.put(fail.getWord(),fail);
								arraySize ++;

							}else if (mainInput.toLowerCase().equals("passed") == true) {

								PassCT pass = new PassCT();
								pass.setInput(mainInput);
								big[arraySize] = pass;
								table.put(pass.getWord(),pass);
								arraySize ++;
